...
Funcionalidade: Remover filme
    ...
    Sendo um gestor de catálogo que encontrou um titulo cancelado/indesejado ou que não tem uma boa aceitação pelo público
    Posso remover este item 

    @rm_movie
    Cenario: Confirmar exclusão
        Dado que "dbz" está noo catálogo
    ...
    Cenario: Cancelar exclusão
        Dado que "10 coisas que eu odeio em você" está noo catálogo
        Quando eu solicito a exclusão
